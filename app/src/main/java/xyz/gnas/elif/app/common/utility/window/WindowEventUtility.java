...
public class WindowEventUtility {
    ...
    private static void bindWindowListener(Class callerClass, Scene scene, WindowEventHandler handler) {
        ...
        if (window != null) {
            window.setOnShown(l -> executeRunner(callerClass, "Error when handling window shown event",
                    handler::handleShownEvent));

            window.focusedProperty().addListener(
                    l -> executeRunner(callerClass, "Error when handling window focused event", () -> {
                        if (window.isFocused()) {

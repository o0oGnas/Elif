package xyz.gnas.elif.app.common;

/**
 * @author Gnas
 * @Description Contains commonly used constants in the application
 * @Date Oct 9, 2018
 */
public class Configurations {
    public static final String SETTING_FILE = "setting.txt";
    public static final String ASCENDING = "KEYBOARD_ARROW_UP";
    public static final String DESCENDING = "KEYBOARD_ARROW_DOWN";
    public static final String BACK = "ARROW_BACK";
    public static final String FORWARD = "ARROW_FORWARD";
    public static final String PAUSE_GLYPH = "PAUSE";
    public static final String RESUME_GLYPH = "PLAY_ARROW";


}